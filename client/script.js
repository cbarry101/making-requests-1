...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/baconbits')
    .then((res) => {
        console.log(res.data)
...
const request = () => {
    axios.get('http://localhost:3000/query-test?name=connor')
    .then((res)=> {
        console.log(res.data)
        let queryShow = document.querySelector('#query-text')
        queryShow.textContent = res.data
        queryShow.style.display = 'block'
    })
}
...
/*
    ...
    1: Send no queries on the URL -- what happened? it displayed "you sent an empty query"

    2: Send more than 1 query on the URL -- what happened? it displayed in the console  that you sent more than one query
*/
...
const createFood = () => {
    ...
    .then((res) => {
        ...
        finalText.textContent = res.data
